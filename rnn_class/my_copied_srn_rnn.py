...
class SimpleRNN:
    ...
    def fit(self, X, learning_rate=10e-1, mu=0.99, reg=1.0, activation=T.tanh, epochs=500, show_fig=False):
        ...
        self.params = [self.We, self.Wx, self.Wh, self.h0, self.Wo, self.bo]
        ...
        def recurrence(x_t, h_t1):
            h_t = self.f(h_t1.dot(self.Wh) + self.bh + x_t.dot(self.Wx))
        ...
        for i in range(epochs):
            ...
            for j in range(N):
                ...
                c, p = self.train_op(input_sequnece, output_sequence)
                cost += c
    ...
    def set(self, We, Wx, Wh, bh, h0, Wo, bo, activation):
        ...
        init_list = [We, Wx, Wh, bh, h0, Wo, bo]
        ...
        def recurrence(x_t, h_t1):
            h_t = self.f(h_t1.dot(self.Wh) + self.bh + x_t.dot(self.Wx))  # chyba byl blad h_t = self.f(self.h_t1.dot(self.Wh) + self.bh + x_t.dot(self.Wx))
            y_t = T.nnet.softmax(h_t.dot(self.Wo) + self.bo)
            return h_t, y_t
...
def train_poetry():
    ...
    rnn.save('RNN_D30_M30_epochs2000_relu_my.npz')


def generate_potery():
    sentences, word2idx = get_robert_frost()

    rnn = SimpleRNN.load('RNN_D30_M30_epochs2000_relu_my.npz', T.nnet.relu)  # sprawdzic czy dziala tez bez
