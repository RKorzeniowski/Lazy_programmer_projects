...
class SimpleRNN:
    ...
    def fit(self, X, Y, learning_rate=10e-1, mu=0.99, reg=1.0, activation=T.tanh, epochs=100, show_fig=False):
        ...
        [h, y], _ = theano.scan(
            fn=recurrence,
            outputs_info=[self.h0, None],#to jest argument to h_t1 i musi byc podany w formie maceierzy chyba
            sequences=thX,#to jest argument do x_t ale jest maceirza wiec zmienia to wyzej
            n_steps=thX.shape[0],
        )
        ...
        for i in range(epochs):
            ...
            for j in range(N):
                ...
                cost += c
                if p[-1] == Y[j, -1]:  # print whole thing to understand
                    n_correct += 1
            print("shape of y ", rout.shape)
