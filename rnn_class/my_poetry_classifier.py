...
class SimpleRNN:
    ...
    def fit(self, X, Y, learning_rate=10e-1, mu=0.99, reg=1.0, activation=T.tanh, epochs=500, show_fig=False):
        ...
        lr = T.scalar('learning_rate')  # adaptive learning rate
        ...
        n_correct_valid = 0
        for j in ragne(Nvalid):
            p = self.predcit_op(Xvalid[j])
            if p == Yvalid[j]:
                n_correct_valid += 1
    ...
    def set(self, Wx, Wh, bh, h0, Wo, bo, activation):
        ...
        self.params = [self.Wx, self.Wh, self.h0, self.Wo, self.bo]
        ...
        self.predcit_op = theano.function(
            inputs=[thX],
            outputs=prediction,
            allow_input_downcast=True,
        )
