...
class AutoEncoders(object):
    ...
    def build(self, D, M):
        ...
        # using the naive formulation for cross entroy will have numerical stability issues if X_hat  =  0 or 1
...
class DNN(object):
    def __init__(self, D, hidden_layers_sizes, K, UnsupervisedModel=AutoEncoders):
        self.hidden_layers = []
        count = 0
        intput_size = D
        for output_size in hidden_layers_sizes:
            ae = UnsupervisedModel(intput_size, output_size, count)
            self.hidden_layers.append(ae)
            count += 1
            intput_size = output_size
        self.build_final_layer(D, hidden_layers_sizes[-1], K)
    ...
    def fit(self, X, Y, Xtest, Ytest, pretrain=True, epochs=1, batch_sz=100):
        ...
        # greedy layer wise autoencoders
        pretrain_epochs = 1
        if not pretrain:
            pretrain_epochs = 0
