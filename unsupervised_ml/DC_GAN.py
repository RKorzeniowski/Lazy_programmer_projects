...
class ConvLayer:
    def __init__(self, name, mi, mo, apply_batch_norm, filtersz=5, stride=2, f=tf.nn.relu):
        ...
        self.W = tf.get_variable(
            ...
            # initializer = tf.contrib.layers.xvier_initalizer(),
            initializer=tf.truncated_normal_initializer(stddev=0.02)  # it cuts all values and redraws them if they are more than 2 standard stddev
        )
...
# i guess this is for generator but then why does it have relu as function
class FractionallyStridedConvLayer:
    def __init__(self, name, mi, mo, output_shape, apply_batch_norm, filtersz=5, stride=2, f=tf.nn.relu):
        # mi = inputfeature map size
        # mo = output feature map size
        # NOTE!!! shape is specified in the OPPOSITE way from the regular conv
...
class DCGAN:
    def __init__(self, img_length, num_colors, d_sizes, g_sizes):
        ...
        # note: by making batch_sz a placegolder we can specify a variable
        # number if samples in the FS-conv operation where we are required
        # to passs in output_shape
        ...
        # get sample images for test time (batch nrom is ifferent)
        ...
        # set up session and variable variables for later
    ...
    def build_discriminator(self, X, d_sizes):
        with tf.variable_scope("discriminator") as scope:
            ...
            for mo, filtersz, stride, apply_batch_norm in d_sizes['conv_layers']:
                # make uo a name used for get_variables
                name = "convlayers_%s" % count
                count += 1
            ...
            # build the cost layer
            return logits

    def d_forward(self, X, reuse=None, is_training=True):
        # encapsulate this cuz we use it twice
        output = X
    ...
    def build_generator(self, Z, g_sizes):
        with tf.variable_scope("generator") as scope:
            ...
            # the first layer of the generator is acutally last
            # so lets revers it
            ...
            for i in range(len(g_sizes['conv_layers'])):
                ...
                mo, filetersz, stride, apply_batch_norm = g_sizes['conv_layers'][i]
                ...
                layer = FractionallyStridedConvLayer(
                    name, mi, mo, output_shape, apply_batch_norm, filetersz, stride, f
                )
            ...
            for j in range(n_batches):
                ...
                if total_iters % SAVE_SAMPLE_PERIOD == 0:
                    ...
                    if samples.shape[-1] == 1:
                        # if color == 1 , we want a 2D image (NxN)
