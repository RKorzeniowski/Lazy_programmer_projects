...
class VariationalAutoencoder(object):
    def __init__(self, D, hidden_layer_sizes):
        ...
        h = DenseLayer(M_in, 2 * M, lambda x: x)
        self.encoder_layers.append(h)

        # czy tu nie bylo przeciganie tego przez forward
        current_layer_value = self.X
        for layer in self.encoder_layers:
            current_layer_value = layer.forward(current_layer_value)
        self.means = current_layer_value[:, :M]
        self.stddev = T.nnet.softplus(current_layer_value[:, M:]) + 1e-6  # added not to cause value too close to 0 cuz it would make singularity
        ...
        # decoder
        # tu jest bez 1 i ostatniego layera w
        self.decoder_layers = []
        M_in = M
        ...
        self.decoder_layers.append(h)  # sigmoid cuz we want bernuli probablities for output

        # przetworzenie layerow
        ...
        #(get some value(number or vector) that can be translated into picture by the decoder)
        # sample from X_hat
        ...
        # takes a sample
        # self.prior_predictive_from_Z = self.rng.binomial(
        #     size=self.prior_predictive_probs_from_Z_input.shape,
        #     n=1,
        #     p=self.prior_predictive_probs_from_Z_input
        # )
