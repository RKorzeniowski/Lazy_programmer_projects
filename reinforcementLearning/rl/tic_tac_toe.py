...
#small modification in 4x4 grid with 3 in a rwo to win
  ...
  def game_over(self, force_recalculate=False):
    ...
    for i in range(LENGTH):
      for player in (self.x, self.o):
        if self.board[i].sum() == player*(LENGTH-1):
          self.winner = player
          self.ended = True
          return True
    ...
    for j in range(LENGTH):
      for player in (self.x, self.o):
        if self.board[:,j].sum() == player*(LENGTH-1):
          self.winner = player
          self.ended = True
          return True
    ...
    for player in (self.x, self.o):
      ...
      if self.board.trace() == player*(LENGTH-1):
        self.winner = player
        self.ended = True
        return True
      # top-right -> bottom-left diagonal
      if np.fliplr(self.board).trace() == player*LENGTH-1:
        self.winner = player
        self.ended = True
        return True
