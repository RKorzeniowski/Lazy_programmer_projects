...
class HiddenLayerBatchNorm(object):
    def __init__(self, M1, M2, f):
        ...
        # FOR TEST
        # self.running_mean = T.(np.zeros(M2))
        # self.running_var = T.(np.zeros(M2))
    ...
    def forward(self, X, is_traning):
        ...
        if is_traning:
            out, batch_mean, batch_invstd, new_running_mean, new_running_var = batch_normalization_train(
                activation, self.gamma, self.beta, running_mean=self.running_mean, running_var=self.running_var)
...
class ANN(object):
    ...
    def fit(self, X, Y, Xtest, Ytest, activation=T.nnet.relu, learning_rate=1e-2, mu=0.9, epochs=15, batch_sz=100, print_period=100, show_fig=True):
        ...
        # for traning
        p_y_given_x = self.forward(thX, is_traning=True)
        ...
        # for testing
        test_p_y_given_x = self.forward(thX, is_traning=False)
        test_prediciton = T.argmax(test_p_y_given_x, axis=1)

        self.predict = theano.function(
            inputs=[thX],
            outputs=test_prediciton,
        )
