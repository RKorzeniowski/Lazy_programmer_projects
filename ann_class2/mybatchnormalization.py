...
class HiddenLayerBatchNorm(object):
  def __init__(self, M1, M2, f):
    ...
    self.M1 = M1
    self.M2 = M2
    self.f = f  # actication function

    W = init_weight(M1, M2).astype(np.float32)  # no bias term becaouse batch normalization make it redundant
    gamma = np.ones(M2).astype(np.float32)
    beta = np.zeros(M2).astype(np.float32)

    # initialized them in tensorflow
  ...
  def forward(self, X, is_training, decay=0.9):
    ...
    activation = tf.matmul(X, self.W)  # <- in tf its a = X.dot(W)

    if is_training:
      batch_mean, batch_var = tf.nn.moments(activation, [0])
      update_running_mean = tf.assign(
          self.running_mean,  # variable you want to update
          self.running_mean * decay + batch_mean * (1 - decay)  # update expression used to update this variable
      )
      update_running_var = tf.assign(
          self.running_var,
          self.running_var * decay + batch_var * (1 - decay)  # its exponentialy smothed average
      )

      with tf.control_dependencies([update_running_mean, update_running_var]):  # we need to do this to update variables that we just created
